...
const LuxuryHeader = ({ storeConfig }) => {
  ...
  const categories = [
    { name: 'New In', path: '/products?collection=new' },
    { name: 'Women', path: '/products?category=women' },
    { name: 'Men', path: '/products?category=men' },
    { name: 'Accessories', path: '/products?category=accessories' },
    { name: 'Sale', path: '/products?collection=sale', highlight: true },
  ];
              ...
              {categories.map((cat) => (
                <Link
                  ...
                  className={`text-[13px] tracking-wider uppercase transition-colors ${
                    cat.highlight ? 'text-red-600 hover:text-red-700' : 'text-gray-800 hover:text-black'
                  }`}
                >
                  {cat.name}
                </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex gap-4">
              ...
              {storeConfig?.social?.facebook && (
                <a href={storeConfig.social.facebook} target="_blank" rel="noopener noreferrer"
                   className="w-10 h-10 border border-white/20 rounded-full flex items-center justify-center hover:bg-white/10 transition">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gray-400">
              <li><Link to="/products?collection=new" className="hover:text-white transition">New Arrivals</Link></li>
