...
async def send_bulk_message(message_data: dict):
    """Send bulk WhatsApp message to customer segment"""
    try:
        segment = message_data.get("segment")
        ...
        sent_count = len(target_customers)
        
        logger.info(f"Would send message to {sent_count} {segment} customers: {message[:50]}")
        
        return {
            "success": True,
            "message": f"Message sent to {segment} segment",
            "sent_count": sent_count
        }
